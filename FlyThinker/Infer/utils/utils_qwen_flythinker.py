...
class GenerationMixin(ContinuousMixin):
    ...
    def _prepare_cache_for_generation(
        ...
        if generation_config.cache_implementation is not None:
            ...
            # print('2',model_kwargs)
        # Use DynamicCache instance by default. This will avoid back and forth from legacy format that
        # keeps copying the cache thus using much more memory
        else:
            # print(dynamic_cache_kwargs)
            model_kwargs[cache_name] = (
    ...
    def generate(
        ...
        **kwargs,
        ...
        # print(f'model_kwargs0 {model_kwargs}')
        ...
        # print(f'model_kwargs1 {model_kwargs}')
        ...
        # print(f'model_kwargs2 {model_kwargs}')
        ...
        # print(generation_config.tolist()))
        # print(f'model_kwargs4 {model_kwargs}')
        ...
        # TODO joao, manuel: remove this in v4.62.0
            ...
            result = self._sample(
                ...
                synced_gpus=synced_gpus,
                streamer=streamer,
                embeds_table = embeds_table,
                is_base = is_base,
                lambda_diff = lambda_diff,
                input_embeds_reasoner = input_embeds_reasoner,
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        if is_base is not None:
            base_llm = is_base.to(model_kwargs['inputs_embeds'].dtype).cuda()
            r1_is_base = True
            
        input_emb_norms = torch.norm(embeds_table.weight, p=2, dim=-1).mean()
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            if step == 0:
                ...
                model_kwargs_r1['inputs_embeds'] = input_embeds_reasoner
                dynamic_cache_kwargs = {"config": reasoner_config}
                model_kwargs_r1["past_key_values"] = (
                    DynamicCache(**dynamic_cache_kwargs)
                )
                ...
                #model_kwargs:dict_keys(['inputs_embeds', 'attention_mask', 'num_logits_to_keep', 'past_key_values', 'use_cache', 'cache_position'])
                model_inputs_r1 = self.prepare_inputs_for_generation(input_ids, **model_kwargs_r1)

                if r1_is_base:
                    outputs_r1 = base_llm(**model_inputs_r1, return_dict=True, output_hidden_states = True)
                else:
                    outputs_r1 = model_forward(**model_inputs_r1, return_dict=True, output_hidden_states = True)
                
                model_kwargs_r1 = self._update_model_kwargs_for_generation(
                    outputs_r1,
                    model_kwargs_r1,
                    is_encoder_decoder=self.config.is_encoder_decoder,
                )
                
                r2_inp_emb_temp = outputs_r1.hidden_states[-1]
                hidden_norms = torch.norm(r2_inp_emb_temp, p=2, dim=-1)
                r2_inp_emb_temp = r2_inp_emb_temp * input_emb_norms / hidden_norms[:,:,None]

                r2_inp_emb = model_kwargs_r2['inputs_embeds']
                if r2_inp_emb.shape[-1] == r2_inp_emb_temp.shape[-1]:
                    r2_inp_emb[:,1:,:] = r2_inp_emb[:,1:,:] + lambda_diff * r2_inp_emb_temp[:,:-1,:]
                else:
                    r2_inp_emb[:,1:,:r2_inp_emb_temp.shape[-1]] = r2_inp_emb[:,1:,:r2_inp_emb_temp.shape[-1]] + lambda_diff * r2_inp_emb_temp[:,:-1,:]
                  
                
                model_inputs_r2 = self.prepare_inputs_for_generation(input_ids, **model_kwargs_r2)
                model_inputs_r2.pop("input_ids")
                model_inputs_r2.pop("inputs_embeds")
                
                outputs_r2 = model_forward(
                ...
                model_kwargs_r2 = self._update_model_kwargs_for_generation(
                    outputs_r2,
                    model_kwargs_r2,
                    is_encoder_decoder=self.config.is_encoder_decoder,
                )

                r2_inp_emb_temp = outputs_r1.hidden_states[-1][:,-1:,:]
                #model_inputs:dict_keys(['cache_position', 'past_key_values', 'input_ids', 'inputs_embeds', 'position_ids', 'attention_mask', 'num_logits_to_keep', 'use_cache'])
                
            else:
                model_inputs_r1 = self.prepare_inputs_for_generation(input_ids, **model_kwargs_r1)
                model_inputs_r2 = self.prepare_inputs_for_generation(input_ids, **model_kwargs_r2)
            
                # print(model_inputs_r1['input_ids'])
                if r1_is_base:
                    outputs_r1 = base_llm(**model_inputs_r1, return_dict=True, output_hidden_states = True)
                else:
                    outputs_r1 = model_forward(**model_inputs_r1, return_dict=True, output_hidden_states = True)
                ...
                r2_inp_emb = embeds_table(model_inputs_r2['input_ids'])
                
                if r2_inp_emb.shape[-1] == r2_inp_emb_temp.shape[-1]:
                    r2_inp_emb = r2_inp_emb + lambda_diff * r2_inp_emb_temp
                else:
                    r2_inp_emb[:,:,:r2_inp_emb_temp.shape[-1]] = r2_inp_emb[:,:,:r2_inp_emb_temp.shape[-1]] + lambda_diff * r2_inp_emb_temp
            ...
            if step == 1:
                ...
                del outputs, outputs_r1, outputs_r2
