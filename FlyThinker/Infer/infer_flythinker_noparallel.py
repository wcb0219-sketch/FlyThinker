...
def parse_args():
    parser = argparse.ArgumentParser(description="SFT Inference with Distributed Setup")
...
def main():
    ...
    Generator_path = './Qwen2.5-7B-Instruct'
    ...
    flythinker = FlyThinker(Generator, reasoner_path = Reasoner_path).cuda()
    ...
    flythinker.eval()
    flythinker = DDP(flythinker, device_ids=[local_rank], output_device=local_rank)
    ...
    for idx, path in enumerate(paths):
        ...
        embeds_table = flythinker.module.Generator.get_input_embeddings()
        embeds_table_x = flythinker.module.Generator.get_input_embeddings()
        ...
        with torch.no_grad():
            ...
            for ff, batch in enumerate(progress_bar):
                for sample in batch: 
                    ...
                    input_ids = batch_tokens.input_ids.cuda()
                    attention_mask = batch_tokens.attention_mask.cuda()
                    
                    input_embeds = embeds_table(input_ids)
                    input_embeds_reasoner = flythinker.module.Reasoner.get_input_embeddings()(input_ids)
                    

                    generated_output = flythinker.module.Generator.generate(
                        ...
                        embeds_table=embeds_table_x,
                        is_base=flythinker.module.Reasoner,
